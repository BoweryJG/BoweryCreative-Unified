...
const clientEmails_js_1 = require("../services/clientEmails.js");
...
router.get('/setup/:domain/:account', async (req, res) => {
    try {
        ...
        const instructions = clientEmails_js_1.clientEmailHelpers.getSetupInstructions(domain, account);
        res.json({
            success: true,
...
router.get('/account/:domain/:account', async (req, res) => {
    try {
        const { domain, account } = req.params;
        const accountConfig = clientEmails_js_1.clientEmailHelpers.getEmailAccount(domain, account);
...
router.get('/signature/:domain/:account', async (req, res) => {
    try {
        ...
        let signature;
        if (format === 'html') {
            signature = clientEmails_js_1.clientEmailHelpers.getHTMLSignature(domain, account);
        }
        else {
            signature = clientEmails_js_1.clientEmailHelpers.getEmailSignature(domain, account);
        }
        res.json({
            success: true,
            account: `${account}@${domain}`,
...
router.get('/domain/:domain/accounts', async (req, res) => {
    try {
        ...
        if (domain === 'gregpedromd.com') {
            const accounts = Object.keys(clientEmails_js_1.gregPedroMD.accounts).map(accountName => {
                const account = clientEmails_js_1.gregPedroMD.accounts[accountName];
                return {
            ...
            res.json({
                success: true,
                domain,
                client: clientEmails_js_1.gregPedroMD.client,
                accounts
            });
        }
        else {
...
router.get('/cindi/setup', async (req, res) => {
    try {
        const instructions = clientEmails_js_1.clientEmailHelpers.getSetupInstructions('gregpedromd.com', 'cindi');
        const textSignature = clientEmails_js_1.clientEmailHelpers.getEmailSignature('gregpedromd.com', 'cindi');
        const htmlSignature = clientEmails_js_1.clientEmailHelpers.getHTMLSignature('gregpedromd.com', 'cindi');
        res.json({
            success: true,
